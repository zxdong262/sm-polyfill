 ...
 * 
 * This polyfill provides SM2 signing/verification using the sm-crypto
...
/**
 * Check if sm-crypto is available
 * @returns {boolean} True if sm-crypto is installed
 */
...
function getSM2() {
  ...
  try {
    ...
    throw new Error(
      'sm-crypto package is required for SM2 polyfill. ' +
      'Install it with: npm install sm-crypto'
    );
  }
}
...
function getSM3() {
  ...
  try {
    sm3Module = require('sm-crypto').sm3;
    return sm3Module;
  } catch {
    throw new Error(
      'sm-crypto package is required for SM3 polyfill. ' +
      'Install it with: npm install sm-crypto'
    );
  }
}
...
function parseOpenSSHPublicKey(publicKeyPem) {
  ...
  // sm-crypto expects the full uncompressed point with 0x04 prefix
  // Q is 65 bytes: 0x04 + X(32) + Y(32)
  if (Q.length !== 65 || Q[0] !== 0x04) {
    throw new Error('Invalid SM2 public key point format');
  }
  
  // Return with 0x04 prefix (sm-crypto format)
  return Q.toString('hex');
}

/**
 * Convert sm-crypto signature (r, s as concatenated hex) to DER format
 * 
 * sm-crypto returns signature as r(32 bytes) + s(32 bytes) = 64 bytes
...
function sign(data, privateKey, hashAlgo) {
  ...
  const sm2 = getSM2();
  const privateKeyHex = parseOpenSSHPrivateKey(privateKey);
...
function verify(data, signature, publicKey, hashAlgo) {
  ...
  // Use sm-crypto polyfill
  const sm2 = getSM2();
  const publicKeyHex = parseOpenSSHPublicKey(publicKey);
